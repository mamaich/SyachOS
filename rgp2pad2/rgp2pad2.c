...
// Мёртвая зона своя почти не нужна: сам драйвер геймпада объявляет flat 256 и
// схлопывает окрестность нуля в точный ноль. Замер на устройстве: 30 с покоя —
// ни одного события, каждое из ~25 отпусканий со стопора вернулось ровно в 0,
// дрейфа нет. Поэтому 250, а не 2500 как у автора: курсор трогается от
// малейшего касания стика.
#define DEF_MOUSE_DEAD 250.0f
